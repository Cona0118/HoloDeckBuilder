...
export default function CardItem({ card, compact = false }: CardItemProps) {
  ...
  return (
    ...
    >
      ...
      >
        {card.imageUrl ? (
          <img src={card.imageUrl} alt={card.name} className="w-full h-full object-cover" draggable={false} loading="lazy" />
        ) : (
          <div className="flex flex-col items-center gap-1 p-2 text-center">
            ...
            <span className="text-[9px] text-gray-600">{card.cardNumber}</span>
          </div>
        )}
      ...
      <div className="px-2 py-2 flex items-baseline justify-between gap-1 w-full">
        <p className="text-[10px] md:text-sm font-semibold text-white leading-tight line-clamp-2 md:truncate min-w-0">{card.name}</p>
