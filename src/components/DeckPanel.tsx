...
// ──────────────────────────────
function DeckStats() {
  const { getActiveDeck, getMainDeckCount, getDeckErrors } = useDeckStore();
  ...
  const mainPct = (mainCount / 50) * 100;

  const colorMap: Record<string, number> = {};
  deck.mainDeck.forEach(({ card, count }) => {
    card.color?.forEach((c) => {
      colorMap[c] = (colorMap[c] ?? 0) + count;
    });
  });
    ...
    <div className="p-3 border-b border-gray-800 flex flex-col gap-3">
      ...
      <div>
        <div className="flex justify-between text-xs mb-1">
          ...
          <span className={mainCount === 50 ? 'text-green-400 font-bold' : 'text-white'}>
            {mainCount} / 50
          </span>
        </div>
      ...
      {mainCount > 0 && (
        <div className="flex gap-3 text-xs">
          <span className="text-emerald-400">홀로멤 {holomemCount}</span>
          <span className="text-sky-400">서포트 {supportCount}</span>
        </div>
      )}
        ...
        <div className="flex flex-wrap gap-1">
          {Object.entries(colorMap).map(([color, count]) => (
            <span
              ...
              style={{
                color: COLOR_ACCENT[color as keyof typeof COLOR_ACCENT],
                borderColor: COLOR_ACCENT[color as keyof typeof COLOR_ACCENT] + '88',
              }}
            >
              {count}
            </span>
